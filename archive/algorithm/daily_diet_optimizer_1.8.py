...
class DailyDietOptimizer:
    def __init__(self, data_path=DATA_PATH):
        print("⚙️ AI 추천 엔진 초기화 중 (v1.8 No Main-Dish Constraint)...")
        ...
        # ❗ [수정됨] 메인 요리 칼로리 제약(300kcal) 제거 -> 모든 후보군 사용
        # self.main_dishes = self.df[self.df['calories'] >= 300].to_dict('records') (삭제)
    ...
    def calculate_nutritional_error(self, combo, target_cal, target_prot, goal_ratios):
        ...
        # 나트륨: 1일 권장량의 1/3을 1끼 상한선으로 (1.2배 여유)
    ...
    def recommend_daily_diet(self, target_cal, target_prot, user_goal, allergies_to_avoid=[], num_simulations=100000):
        ...
        print(f"🔄 시뮬레이션 시작: 1끼 목표 (Cal:{target_cal}, Prot:{target_prot}) | 품목 수: 1~4개")
        ...
        for _ in range(num_simulations):
            ...
            # ✅ 브랜드 제약 (유지됨): 동일 브랜드 상품은 2개까지만 허용 (3개 이상 시 탈락)
...
if __name__ == "__main__":
    ...
    for i in range(MEALS_COUNT):
        ...
        if isinstance(meal_result, str):
            ...
            all_meal_results.append(meal_result[0]) # 1순위 식단 선택
    ...
    if any(all_meal_results):
        ...
        print("\n\n==================================================")
        print(f"🥗 [AI 추천 결과 v1.8] 3끼 균등 분배 (제약 해제)")
        print("==================================================")
