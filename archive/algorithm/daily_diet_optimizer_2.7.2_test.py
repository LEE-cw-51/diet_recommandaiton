...
# -----------------------------------------------------------
try:
    # Windows: 'Malgun Gothic', Mac: 'AppleGothic' 등 본인 환경에 맞게 수정
...
# [v2.7.2] 당류 제한 비율 (하루 총 칼로리 대비)
SUGAR_RATIOS = {
    "다이어트": 0.05,  # 5% 제한 (매우 엄격)
    "건강관리": 0.10,  # 10% 제한 (WHO 권장)
    "근육증가": 0.10   # 10% 제한
}
...
class DiversityManager:
    """
    [v2.7.1 Logic 유지] 통합 임베딩 (Integrated Embedding)
    """
    def __init__(self):
        self.name_features = [
    ...
    def create_vector(self, item):
        ...
        for feature in self.allergy_features:
            vector.append(1 if (feature in allergens or feature in name) else 0)
...
class DailyDietOptimizer:
    def __init__(self, data_path=DATA_PATH):
        print("⚙️ AI 추천 엔진 초기화 중 (v2.7.2: Sugar Restriction Added)...")
    ...
    # [v2.7.2 수정] sugar_limit 인자 추가 및 당류 페널티 로직 적용
    def calculate_nutritional_error(self, combo, target_cal, target_prot, target_fat, sugar_limit, goal_ratios, prot_min_factor=0.95, cal_range=0.15):
        ...
        fat_penalty = 0
        if total_fat > target_fat: fat_penalty = ((total_fat - target_fat) / target_fat) ** 2 * 2.0
        ...
        if total_sugar > sugar_limit:
            # 한도를 초과하면 가파르게 페널티 부여
            sugar_penalty = ((total_sugar - sugar_limit) / sugar_limit) ** 2 * 3.0 
            
        error_score = np.sqrt(cal_error + prot_error + fat_penalty + sodium_penalty + sugar_penalty)
        ...
        # 당류 유효성 체크 (소프트 리미트: 약간 초과 허용하되 점수는 나쁘게)
        # 1.5배 이상 초과하면 아예 제외
        is_sugar_valid = (total_sugar <= sugar_limit * 1.5)

        return error_score, total_cal, total_prot, total_carbs, total_fat, total_sodium, total_sugar, is_sodium_valid, is_protein_min_met, is_cal_valid, is_sugar_valid
    ...
    def recommend_daily_diet(self, target_cal, target_prot, target_fat, user_goal, allergies_to_avoid=[], excluded_codes=None, excluded_brands=None, num_simulations=20000, **kwargs):
        ...
        # [v2.7.2] 당류 제한량 계산 (g) = (해당 끼니 목표 칼로리 * 제한 비율) / 4kcal
        ...
        for _ in range(num_simulations):
            ...
            # [v2.7.2] 오차 계산 함수에 meal_sugar_limit_g 전달
            error, tot_cal, tot_prot, tot_carbs, tot_fat, tot_sodium, tot_sugar, is_sodium_valid, is_protein_min_met, is_cal_valid, is_sugar_valid = self.calculate_nutritional_error(
                combo, target_cal, target_prot, target_fat, meal_sugar_limit_g, goal_ratios, prot_min_factor, cal_range
            )

            # [v2.7.2] is_sugar_valid 조건 추가
            if is_protein_min_met and is_cal_valid and is_sodium_valid and is_sugar_valid:
                valid_combinations.append({
                    ...
                    'fat': tot_fat,
                    'sodium': tot_sodium,
                    'sugars': tot_sugar, # 결과에 당류 추가
                    'sugar_limit': meal_sugar_limit_g, # 한도 정보 추가
                    'error': error,
...
def run_single_simulation(user_profile):
    ...
    for i in range(MEALS_COUNT):
        ...
        if not isinstance(meal_result, str):
            ...
            current_status['sugars'] += best_combo['sugars'] # 당류 누적
...
if __name__ == "__main__":
    ...
    print(f"\n🚀 [Parallel] {NUM_USERS}명 시뮬레이션 시작 (v2.7.2: Sugar Restriction 5~10%)")
    ...
    for res in success_results:
        ...
        # 당류 한도 대비 섭취율 (%)
        sugar_usage = (res['sugars'] / res['sugar_limit']) * 100
        sugar_usage_rates.append(sugar_usage)
    ...
    print(f"🍭 평균 당류 한도 소진율: {avg_sugar_rate:.1f}% (100% 미만 권장)")
    ...
    if success_cnt > 0:
        try:
            ...
            plot_distribution(axes[1, 0], sugar_usage_rates, "Sugar Limit Usage", "Usage (%)", color='orange') # 당류 그래프 추가
    ...
    if success_cnt > 0:
        ...
        for idx, sample in enumerate(random_samples):
            ...
            sugar_ratio_print = SUGAR_RATIOS.get(u['goal'], 0.10) * 100
            print(f"\n[Sample {idx+1}] {u['weight']}kg | {u['gender']} | {u['goal']} | Target: {u['target_cal']}kcal")
            print(f"   🍭 당류 제한: 하루 {sample['sugar_limit']:.1f}g 미만 ({sugar_ratio_print:.0f}%)")
            for m_idx, meal in enumerate(sample['results']):
                print(f"   🥣 Meal {m_idx+1} [{meal['brand']}] ({meal['calories']:.0f}kcal, Sugar {meal['sugars']:.1f}g)")
                for item in meal['combo']:
                    print(f"       - {item.get('menu_name', item.get('식품명'))}")
            print(f"   👉 Total: {sample['price']:,}원 | Sugar {sample['sugars']:.1f}g (Usage: {sample['sugars']/sample['sugar_limit']*100:.1f}%) | Div {sample['avg_diversity']:.2f}")