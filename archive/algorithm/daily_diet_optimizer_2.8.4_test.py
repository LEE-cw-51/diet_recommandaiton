...
class DailyDietOptimizer:
    def __init__(self, data_path=DATA_PATH):
        print("⚙️ AI 추천 엔진 초기화 중 (v2.8.9: Compromise Selection Added)...")
    ...
    def recommend_daily_diet(self, target_cal, target_prot, target_fat, user_goal, allergies_to_avoid=[], excluded_codes=None, excluded_brands=None, **kwargs):
        ...
        for _ in range(max_brand_retries):
            ...
            try:
                ...
                final_results = []
                objective_data = [] # Normalize를 위한 F 데이터 저장
                
                for i, x in enumerate(res.X):
                    # [V2.8.9] Feasibility check: NSGA-II는 제약을 위반한 해를 반환할 수 있으므로, 제약 충족 여부 확인
                    ...
                    final_results.append({
                        ...
                        'objectives': res.F[i] # 목적 함수 값 저장
                    })
                
                if not final_results:
                    return "❌ 최적해 발견 실패 (제약 충족 해 없음)"

                # [V2.8.9] 최종 선택 로직 변경: Compromise Solution 선택
                ...
                # 나머지 결과를 포함하여 가격 순으로 정렬하는 대신, 타협안을 첫 번째에 두고 반환
...
if __name__ == "__main__":
    ...
    print(f"\n🚀 [Parallel] {NUM_USERS}명 시뮬레이션 시작 (v2.8.9: Compromise Selection)")
