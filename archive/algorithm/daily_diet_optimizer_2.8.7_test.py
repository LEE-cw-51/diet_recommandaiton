...
# [v2.8.7] Problem 정의 (Fat Limit Constraint 추가)
class DietOptimizationProblem(ElementwiseProblem):
    def __init__(self, mains, sides, drinks, target_cal, target_prot, target_fat, sugar_limit, div_manager, prot_min_factor, cal_range_factor, fat_max_factor):
        self.mains = mains
        self.sides = sides
        self.drinks = drinks
        ...
        self.fat_max_factor = fat_max_factor # [추가] 지방 상한 계수
        ...
        # [수정] n_constr=4 (당류, 단백질, 칼로리, 지방)
        super().__init__(n_var=4, n_obj=3, n_constr=4, xl=xl, xu=xu, type_var=int)
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        # [추가] G4: Fat Limit Constraint (Target의 130% 초과 시 위반)
        g4 = total_fat - (self.target_fat * self.fat_max_factor)
...
class DailyDietOptimizer:
    def __init__(self, data_path=DATA_PATH):
        print("⚙️ AI 추천 엔진 초기화 중 (v2.8.7: Fat Constraint Added)...")
    ...
    def recommend_daily_diet(self, target_cal, target_prot, target_fat, user_goal, allergies_to_avoid=[], excluded_codes=None, excluded_brands=None, **kwargs):
        ...
        fat_max_factor = kwargs.get('fat_max_factor', 1.3) # [추가] 기본값 130%
        ...
        for _ in range(max_brand_retries):
            ...
            try:
                problem = DietOptimizationProblem(
                    ...
                    prot_min_factor, cal_range_factor, fat_max_factor # [추가] 인자 전달
                )
                ...
                sorted_results = [final_results[i] for i in sorted_indices]
                
                return sorted_results[:NUM_ALTERNATIVES]
...
def run_single_simulation(user_profile):
    ...
    for i in range(MEALS_COUNT):
        ...
        meal_result = optimizer_instance.recommend_daily_diet(
            ...
            prot_min_factor=0.95, cal_range_factor=0.05, fat_max_factor=1.30
        )
        
        # 2차 시도 (조건 완화 70%, 10% 칼로리 허용, 지방 140%)
        if isinstance(meal_result, str):
            meal_result = optimizer_instance.recommend_daily_diet(
                target_cal=target_cal, target_prot=target_prot, target_fat=target_fat,
                user_goal=user_profile['goal'], allergies_to_avoid=user_profile['allergies'],
                excluded_codes=excluded_codes, excluded_brands=excluded_brands,
                prot_min_factor=0.70, cal_range_factor=0.10, fat_max_factor=1.40
            )

        # 3차 시도 (브랜드 제한 해제 + 조건 완화)
        if isinstance(meal_result, str):
            meal_result = optimizer_instance.recommend_daily_diet(
                target_cal=target_cal, target_prot=target_prot, target_fat=target_fat,
                user_goal=user_profile['goal'], allergies_to_avoid=user_profile['allergies'],
                excluded_codes=excluded_codes, excluded_brands=set(),
                prot_min_factor=0.70, cal_range_factor=0.15, fat_max_factor=1.50
            )
...
if __name__ == "__main__":
    ...
    print(f"\n🚀 [Parallel] {NUM_USERS}명 시뮬레이션 시작 (v2.8.7: Fat Constraint Added)")
    ...
    if success_cnt > 0:
        ...
        for m_idx, meal_alternatives in enumerate(sample['results']):
            print(f"\n   🥣 Meal {m_idx+1} Alternatives [{meal_alternatives[0]['brand']}]")
            for alt_idx, meal in enumerate(meal_alternatives):
                tag = "[SELECTED]" if alt_idx == 0 else f"[Option {alt_idx}]"
                print(f"       {tag} | Price: {meal['price']:,.0f}원 | Div: {meal['diversity_score']:.2f}")
                for item in meal['combo']: 
                    item_tag = item.get('category_tag', 'ETC')
                    print(f"           [{item_tag}] {item.get('menu_name', item.get('식품명'))}")
            
        print(f"\n   👉 Total Price (Based on SELECTED): {sample['price']:,}원 | Sugar {sample['sugars']:.1f}g | Avg Div {sample['avg_diversity']:.2f}")python algorithm/daily_diet_optimizer_2.8.8_test.py