...
class DailyDietOptimizer:
    def __init__(self, data_path=DATA_PATH):
        print("⚙️ AI 추천 엔진 초기화 중 (v1.4 - 최종 로직)...")
    ...
    def calculate_nutritional_error(self, combo, target_cal, target_prot, goal_ratios):
        ...
        is_sodium_valid = (total_sodium <= SODIUM_MAX_LIMIT) # 나트륨 2500mg 이하
    ...
    def recommend_daily_diet(self, target_cal, target_prot, user_goal, meals_count=3, allergies_to_avoid=[], num_simulations=100000):
        ...
        if len(safe_dishes) < meals_count:
             return "❌ 알레르기 필터링 후 남은 메뉴가 부족합니다. 필터를 완화하거나 끼니 수를 줄여주세요."

        print(f"🔄 시뮬레이션 시작: 목표='{user_goal}', {meals_count}끼 (알레르기 필터 적용)")

        valid_combinations = []
        
        for _ in range(num_simulations):
            # 중복 없는 조합 생성
            combo = random.sample(safe_dishes, k=meals_count)
            ...
            # [핵심 v1.4 수정] 단백질 상한선 제거 (단, 90% 하한선 유지)
            is_target_met = (target_cal * 0.85 <= tot_cal <= target_cal * 1.15) and \
                            (tot_prot >= target_prot * 0.90) 
            ...
            if is_target_met and is_ratio_valid and is_sodium_valid and is_diverse:
...
if __name__ == "__main__":
    ...
    print(f"📊 [사용자 설정] 목표 단백질: {USER_PROT}g | 제외 알레르기: {USER_ALLERGIES}")
    ...
    if isinstance(result, str):
        ...
        print("\n==================================================")
        print(f"🥗 [AI 추천 결과] {MEALS}끼 식단 (v1.4 - 최종) | 목표: {USER_GOAL}")
        print("==================================================")
